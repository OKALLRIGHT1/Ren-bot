...
def _load() -> Dict[str, Any]:
    if not DELEGATE_TASKS_PATH.exists():
        return {"tasks": {}}
    try:
        with DELEGATE_TASKS_PATH.open("r", encoding="utf-8") as fh:
            data = json.load(fh)
...
def _save(data: Dict[str, Any]) -> bool:
    try:
        DELEGATE_TASKS_PATH.parent.mkdir(parents=True, exist_ok=True)
        tasks = data.get("tasks", {})
        if isinstance(tasks, dict) and len(tasks) > MAX_TASKS:
            ...
            data["tasks"] = tasks
        with DELEGATE_TASKS_PATH.open("w", encoding="utf-8") as fh:
