...
def classify_worker_exit(exit_code):
    if exit_code == 0:
        return "stop"
    if exit_code == RESTART_EXIT_CODE:
        return "restart"
    if exit_code == CORE_ALREADY_RUNNING_EXIT_CODE:
...
def main():
    ...
    while True:
        try:
            ...
            action = classify_worker_exit(exit_code)
            ...
            # 情况3: 已有核心进程，但当前守护进程还没有接管；等待核心退出后重试
            elif action == "wait_for_core":
                print("⏳ [守护进程] 已有核心实例在运行，等待接管...")
                time.sleep(5)
                continue
                
            # 情况4: 异常崩溃 (Exit Code != 0)
            else:
