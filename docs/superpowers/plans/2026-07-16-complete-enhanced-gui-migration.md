...
|---|---|---|---|---|---|---|---|---|
| 1 | Main chat | `app.py` tray chat | `services/chat_service.py`, GUI WS | WS hello/command/log/status/config/character | `ChatPage` | history query optional polish | WS + chat page tests | available |
...
| 3 | Overview dashboard | tray status chips | app runtime | `GET /dashboard` | `OverviewPage` | richer typed chips optional | dashboard contract | available |
| 4 | Models & routing | settings llm/provider + routing overview | `modules/model_catalog.py`, `services/gui_api/models_service.py` | `GET /models`, upsert/delete model/provider, save router (+ legacy raw settings) | `ModelsPage` structured | connection-test optional polish | model service + page unit tests | available |
| 5 | Characters | character editor | `modules/character_manager.py`, `services/gui_api/characters_service.py` | structured list/get/upsert/delete/activate + costume upsert/delete/wear + costume-meta/preview (+ legacy raw) | `CharactersPage` structured | TTS test play optional polish | character service + page unit tests | available |
| 6 | Theme / colors | settings color page | runtime_settings palette + `services/gui_api/theme_service.py` | `GET/POST /theme` | `ThemePage` | console color full matrix polish optional | theme service + page unit tests | available |
| 7 | Plugins | plugin manager dialog | `modules/plugin_manager.py` + `services/gui_api/plugins_service.py` | list/toggle/reload/config/schema/settings form + secret mask | `PluginsPage` schema form + JSON advanced | model_queue picker polish optional | plugin enrichment service + page unit tests | available |
| 8 | Info sources | info_sources_page | `services/info_sources/*` + `services/gui_api/info_sources_service.py` | list/get/save endpoint, token SecretUpdate, draft, test | `InfoSourcesPage` structured | optional provider create UI polish | info source service + page unit tests | available |
| 9 | Knowledge base | knowledge manager + import wizard | `modules/memory/knowledge_store.py` + `services/gui_api/knowledge_service.py` | dirs/stats/search/import/rebuild/delete-dirs/learn/create-doc | `KnowledgePage` | chat-record import wizard optional polish | knowledge service + page unit tests | available |
| 10 | Expression library | expression_library_manager | `memory_sqlite.expression_patterns` + `services/gui_api/expression_library_service.py` | list/get/upsert/delete/set-enabled/runtime | `ExpressionLibraryPage` | optional bulk import/export parity | expression service + page unit tests | available |
...
| 13 | Memory center | memory_editor | `modules/memory_sqlite.py` / advanced_memory / `services/gui_api/memory_service.py` | core list/upsert/delete/category + vector status/rebuild/embedding test + legacy items/episodes/transcript | `MemoryPage` core/vector + legacy tabs | profile overview optional polish | memory service + page unit tests | available |
