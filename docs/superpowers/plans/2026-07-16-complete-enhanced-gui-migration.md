...
| 4 | Models & routing | settings llm/provider + routing overview | `modules/model_catalog.py`, custom_models.json | raw `GET/POST /settings/custom_models` | Settings JSON tab | `GET/POST /models`, providers, test-connection, router chains, embedding selection | model API + page | raw-json-only |
| 5 | Characters | character editor | `modules/character_manager.py` | raw characters + costume-meta/preview | Settings JSON + limited chat costume | structured character CRUD, activate, costume CRUD, motion/expression map, TTS test | character API + page | raw-json-only |
