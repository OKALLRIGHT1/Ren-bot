...
- Explicit web-search requests send one short, topic-aware acknowledgement
...
per incoming request. A small local formatter removes leading search verbs and
trailing phrases such as "latest information", then selects a concise template
from the request text. This avoids a fixed robotic sentence without adding
model latency or another failure point. The acknowledgement is transient: it is
shown and spoken but is not written to the chat transcript or long-term memory.
It is not merged into the final answer. This keeps the two messages semantically
distinct and prevents a model-generated promise from being mistaken for task
completion.
...
- Assert that the acknowledgement includes the request topic, is not the fixed
  sentence `好，我查一下`, and is absent from assistant transcript events.
